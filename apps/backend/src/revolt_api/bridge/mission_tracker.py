...
class MissionExecutionTracker:
	...
	def __init__(self) -> None:
		"""Start with nothing tracked; state is populated by track()/record_waypoint_list_echo()."""
	...
	def tracked_mission_id(self) -> str | None:
		"""The mission id currently being tracked, if any."""
	...
	def tracked_total_count(self) -> int:
		"""The tracked mission's total waypoint count, snapshotted at track() time."""
		return self._tracked_total_count

	def track(self, mission_id: str, total_count: int) -> None:
		"""Start deriving live execution status for this mission.

		Current waypoint and progress are derived from subsequent /waypoint_list echoes. Called
		by the start endpoint before publishing, not after, so the echo that resolves
		publish_and_await_ack's own wait attributes correctly to this mission rather than
		whatever was tracked previously. Defaults to "starting" rather than "active" since the
		ack hasn't landed yet at this point.
		"""
	...
	def set_state_if_tracked(self, mission_id: str, state: MissionExecutionState) -> bool:
		"""Update the tracked state if mission_id matches the currently tracked mission.

		Reports whether it applied; a no-op (returns False) otherwise. Only one mission is
		tracked at a time by design, so this guards callers against broadcasting under the
		wrong id.
		"""
	...
	def record_waypoint_list_echo(self, waypoints: list[SimWaypoint]) -> None:
		"""Store the vessel's latest /waypoint_list echo as the current queue snapshot."""
	...
	def pop_resume_point(self, mission_id: str) -> list[SimWaypoint] | None:
		...
		Called on a successful resume-start (the snapshot has now been used) and on terminate
		(no resume should survive an abort).
		"""
	...
	def clear_resume_cache(self) -> list[str]:
		...
		Called when a publish to /update_waypoint_list replaces the vessel's entire queue, since
		any previously-paused mission's snapshot no longer reflects reality.
		"""
