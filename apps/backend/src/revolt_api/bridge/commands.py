"""Allow-listed ROS2 introspection commands exposed to the GUI's command console.

See the module-level comment on COMMANDS below for why this is introspection-only.
"""
...
class CommandKind(enum.StrEnum):
	"""How a command is executed: a rosapi service call, or a cached topic echo."""
...
class CommandParamSpec:
	"""One parameter a command accepts."""
...
@dataclass(frozen=True)
class CommandSpec:
	"""One entry in the COMMANDS allow-list."""
...
# names/types below are verified against RobotWebTools/rosbridge_suite's rosapi_msgs/srv
# definitions and rosapi_node's actual service handlers (ros2 branch), including
# get_param_names' "<node>:<param>" format and get_param's matching ":"-split parsing.
